...
// JSON.parse(window.localStorage.getItem('stories-array'))
...
function addButtonFunctionality() {
    ...
    for (let i = 0; i < buttonsArray.length; i++) {
        buttonsArray[i].addEventListener('click', () => {
            ...
            const storyText = stories[i].text;
            const paragraph = `<p>${storyText}</p>`;
            const image = document.createElement('img');
            image.src = stories[i].image;
            image.alt = 'image not available';
            image.loading = 'lazy';
            image.classList.toggle('large-image');
            storyContainer.parentElement.appendChild(image);
